 ...
 * be detail with no decision attached. The hold is `ANALYSING_MS`, stated beside the words it
 * paces and reaching the dialog only through `analysingStepMs()` — so the component names
 * neither the total nor a step, the rule `BuildRunDialog` keeps with `BUILD_STAGE_MS`.
 */
expect(
  'and the hand-off holds on a constant it does not restate',
  /export const ANALYSING_MS = 10_000/.test(codeOnly(runtimeBuildSrc)) &&
    !/ANALYSING_MS/.test(runtimeDialogCode) &&
...
/*
 * The steps are the copy's and they are cut out of the one hold rather than each carrying a
 ...
 */
expect(
  'and the steps are the copy’s, cut out of that one hold',
  ...
  'analysing is a list in src/data/, and no step is written into the component',
)
/*
