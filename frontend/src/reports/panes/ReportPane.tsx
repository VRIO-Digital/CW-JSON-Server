...
import { BLOCK_TAG, blockSig, instantiate, isMeasure, kpiOrder, kpiSpec } from '../lib/blocks';
...
export function ReportPane({
  ...
  function toolbarFor(b: Block) {
    ...
    /*
     * The two controls every block has, kept separately so a case can order them.
     *
     * `common` still puts them where they always were — move, then delete, after the block's
     * own controls — so no other block type changes. The chart case places `moveBtns` first
     * instead, which is the order the package's prototype uses: where it sits, what it draws,
     * what it draws *of*, then delete.
     */
    const moveBtns = (
      <MoveSeg
        canUp={i > 0}
    ...
    const deleteBtn = <RemoveBtn onRemove={() => remove(b.id)} />;
    const common = (
      <>
        <ToolbarDivider />
        {moveBtns}
            ...
            <button
              onClick={(e) =>
                open(
                  ...
                  <TogglePicker<KpiKey>
                    title="Which tiles?"
                    options={kpiOrder().map((k) => ({ key: k, label: kpiSpec(k)?.label ?? k }))}
                    selected={b.kpis ?? []}
      ...
      case 'chart':
        return (
          <>
            ...
            <ToolbarDivider />
            {/*
             * Three forms, and each one draws. `line` was offered by the package's own
             * prototype but did not exist here at any layer — `ChartType` was
             * `'bar' | 'column'`, `ChartBlock` had no branch for it, and the toolbar had no
             * button — so the form the reader wanted was simply unavailable.
             *
             * `bar` is the fallback rather than a value to match, because a block whose
             * `chartType` is absent still has to render something.
             */}
            <span className="seg">
              {(['bar', 'column', 'line'] as const).map((form) => (
                <button
                  key={form}
                  className={(b.chartType ?? 'bar') === form ? 'on' : ''}
                  onClick={(e) => {
                    e.stopPropagation();
                    patch(b.id, { chartType: form });
                  }}
                >
                  {form === 'bar' ? 'Bar' : form === 'column' ? 'Column' : 'Line'}
                </button>
              ))}
            </span>
            <ToolbarDivider />
            {/* What the chart is *of*. Named for what it does rather than for the maths —
                "∑ Penalty exposure" read as a total, which is not what picking it changes. */}
            <button
              onClick={(e) =>
                open(
                  e,
