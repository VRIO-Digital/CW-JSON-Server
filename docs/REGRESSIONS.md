...
## Accept all on the relations dialog, and a `confirmed_by` nobody had earned (2026-09-11)

**Two requests, one field.** First: every relation in CAPEX's Data Modeling tab should read *Curated
by AI*. Then: keep an **Accept all** at the top of that dialog.

**The label is not stored.** It is read off `confirmed_by` on each stored relationship — an address
means somebody accepted it and the row reads *Confirmed by you*; `null` means nobody has, and the
row reads *Curated by AI* with Accept offered. 52 of `db.CAPEX.json`'s 59 relationships carried
`"confirmed_by": "adaeze.okonjo@vriodigital.com"`, stamped in an earlier session, so 52 rows
credited an acceptance that reader had not made — **the exact fault `confirmed_by` was added to
fix**, back again in the data rather than in the code.

**Fix** — `backend/scripts/unaccept-data-model.js` (`npm run unaccept:data-model -- CAPEX`) clears
the field to `null` — never deletes the key — on every relationship *and* every entity of a named
dataset. A script rather than a hand-edit because the document's `_meta` forbids one; entities are
cleared in the same run because `tableDeclarationState` reads an entity-level `confirmed_by`, and a
table header reading *Confirmed by you* over rows reading *Curated by AI* is the
panel-arguing-with-itself fault this repo records elsewhere. Verified by re-nulling the same fields
on `git show HEAD:backend/db.CAPEX.json` and deep-comparing: `confirmed_by` was the only thing that
moved.

**Then the bulk act, and the interesting part is the reason it had been refused.** CLAUDE.md said
*"the pending dialog's Accept all has no twin here: there is no bulk act on a stored declaration,
and deleting nineteen at once would be the least reversible button in the tab."* That reasoning is
about **deletion**. Accepting in bulk is the opposite shape — it writes a name onto rows that
already exist, changes nothing about the schema, and every row keeps its own Reject as the way back
out. So the additive half landed and *Reject all* is asserted **absent** beside it: the argument did
not go away, it turned out to be about the other half. **A rule that rules out two acts at once is
worth re-reading before assuming it rules out both.**

**It shipped as a per-row sequence first, and that was reported from use within the hour** — *"when
the user clicks accept all then with accepting one by one it should bulk accept"*. The run was
`acceptRelation` in a loop, re-reading the entities between writes because `relationshipWrites`
builds from the owning entity's current array. Every part of that was correct and the shape was
wrong:

- **59 requests, 59 validations and 59 document commits for one decision.**
- **A refusal partway through left half the list accepted**, with nothing on screen saying which
  half — the one outcome a reader cannot tell from a bug.
- **And it collapsed five declarations as a side effect.** `relationshipWrites` treats a re-declared
  join as an *edit* of the existing one, which is right for an edit; CAPEX ships five exact
  duplicates, so accepting the list one row at a time took *Relations · 59* down to 54 while it ran.
  Nothing errored. **A write path built for one act acquires that act's rules, and they are not
  automatically the right rules for a second act reusing it.**

**Fix** — `POST /data-model/relationships/accept`. The scope is resolved across every entity
*before* anything is written and landed in a single `commitDb`: all of them or none, the arrangement
the multi-dictionary schema upload already has. It writes `confirmed_by` on rows that already exist
and touches nothing else, which is what lets it be a route of its own without becoming a second
answer to what a declaration is — `POST /data-model/entities` still owns every other field.

Four rules on the route, each guarding something quiet:

- **It fills a null `confirmed_by` and never rewrites one**, so a row somebody else accepted keeps
  their name. The `studioPublishedBy` lesson mirrored: there the hazard was a record that was only
  ever set, here it is one that is always overwritten.
- **Scope is both ends of the relationship**, exactly as `declaredRelationshipsFrom` requires for the
  list, so the run cannot accept a row the dialog never showed and the button's count stays a number
  about the same set.
- **Absent `table_keys` means every table in the dataset; `[]` is refused** — accepting nothing and
  accepting everything are opposite requests, the rule `POST …/profile-mail-documents` keeps for its
  own `objects`.
- **Nothing undecided is a refusal naming what it found**, never a 200 reporting zero: a run that
  committed nothing and answered ok reads as an act that happened.

**Guard** — `check-docs`: *the relations dialog accepts in bulk in one commit, counts the undecided,
and has no Reject all*. It asserts the loop is **gone** from the tab as well as the call being
there — a call and a loop can coexist, and only the absence says the act is atomic — plus exactly
one `commitDb` in the route, the resolve happening before it, both-ends scope, the null-only write,
the empty-scope refusal, the server's count in the message, and no *Reject all*. Break-tested ten
ways; all ten turn it red.

**Verified against a live server**, on a copy of the document in a scratch directory: 7 undecided
accepted in **one 36ms request**, the 52 names already there left alone, all **59 items preserved**
with the duplicates intact, and the second press refused with *"all 59 stored relation(s) in scope
have already been accepted"*. An empty `table_keys` and a missing `as` are both refused.

**Two process notes worth more than the feature.**

**The first break test reported MISSED against a correct guard**, for the reason already on record:
the mutation searched for `\n` in a CRLF file and never landed. Written CRLF-aware it turned the
claim red immediately — and the harness now *says* when a mutation does not land rather than
counting it as an uncaught one. The claim's own `!/Reject all/` also had to go through `codeOnly`,
because both files explain in prose why there is none: the self-documenting-file trap, for the sixth
time.

**And a `check-docs` claim written through the Bash tool broke the file.** A heredoc ate one
backslash level, so `[\s\S]*?\n` in the source arrived as a real newline inside a regex literal and
`node check-docs.mjs` died with *"Invalid regular expression: missing /"* — the whole run gone, which
is the failure mode where every break test reports MISSED and correct guards look broken. It also
