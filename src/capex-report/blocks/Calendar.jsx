...
/*
 * The host's own way of opening a report, in place of a `react-router` `<Link>`.
 *
 * The standalone app routes `/reports/:slug`; the host has one `/reports` address and keeps which report
 * is open in a store, so the original link navigated to nothing — and crashed `renderToString` for want
 * of a router. `useCapexHost` is the seam; standing alone its default is a no-op, which makes the chip
 * render as the disabled button this block already draws when it cannot resolve a drill target.
 */
import { useCapexHost } from '../host.tsx'
/*
 * The report's own spec, for the labels this block prints.
 *
 * The standalone app imported the whole 1.5MB fixture for this. Hosted, the specs ride on the served
 * payload and are hydrated into `specs.js` before anything renders — the same move
 * `src/reports/data.ts` makes, and for the same reason: a fixture compiled into the bundle is the one
 * thing on screen the bucket cannot change.
 */
import { reportById } from '../specs.js'
...
function ProjRow({ p, drill }) {
  /* The host's seam — see the import note. Standing alone this is the no-op host. */
  const host = useCapexHost()
  ...
  /* A button rather than an anchor, because the host opens the report in place — there is no address to
     put in an href. The coordinate rides along so the destination can say which one its run holds. */
  return (
    <button className={'calPrI' + (est ? ' est' : '')} type="button" title={title}
            onClick={() => host.openReport(target.id, { param: drill.param, value: String(value) })}>
      {inner}
    </button>
  )
}
