...
### Two datasets, and the process holds however many there are

**A dataset is a prefix, and there are two.** `EPA/` is the primary and holds everything described
below. `CAPEX/` is the second and is **real**: Northline Water Group, 3 GCP projects, 1 Drive, 60
capital projects, 31 contracts, 22 change orders and 3 report definitions, generated by
`capex_demo_data_package_2026-08-18` against this repo’s own schema. It was seeded and then removed
once; what came back is a document behind the name, which is why `DATASETS` and the document always
move together — every dataset in that list is read at boot, so a name with nothing behind it stops
the boot.

**CAPEX’s document is *adopted*, not seeded, and that is a different script.**
`npm run seed:dataset -- <NAME>` writes the primary’s structure with the primary’s rows removed —
right for a dataset that has none, and it would throw this one away.
`npm run adopt:dataset -- CAPEX "<file>"` carries the package’s document through unchanged and fills
only what a package generator cannot know. Three of its decisions are load-bearing:

- **The tenant’s directory travels as one unit** — `auth_roles`, `settings`, `google_account`. They
  answer "who exists", they cross-reference each other, `MERGE_PLAN` marks all three `primary`, and
  `validateDb` enforces the cross-reference: every `settings.users[].role_id` must name one of the
  **document’s** `auth_roles`. Carrying half the pair is what breaks it, and it did — the CAPEX
  package generated its own four personas (`analyst`, `architect`, …) and every carried user then
  named a role the document did not have. The package’s own set is **set aside and said so** on the
  way out; a dataset’s own personas would be a second answer to who has signed in, and the login
  resolves a persona against the tenant directory.
- **A missing collection is a refusal**, never filled from the primary. Borrowing EPA’s projects or
  rosters is the single failure the dataset split exists to prevent.
- It is deliberately **not** "every `primary` key comes from the primary". `whatif` is `primary` too,
  and CAPEX has its own facility, its own candidate loads and its own appetite line. `MERGE_PLAN`
  answers what `both` *displays* for a single-valued key; it never says a dataset may not hold its own.

**Two things the adopt had to correct, both stated rather than silent.** The package ships
`reports.governance.audit.copy.not_enforced` as `null` — the sentence `validateDb` checks *by phrase*
because a page that lets somebody author a restriction and stays quiet about enforcement is implying
one runs; the primary’s is carried, since it is a statement about **this app’s behaviour** and equally
true of every dataset it serves. And four of its five recorded sanity checks list `t1`…`t4` in
`edges_used` — the package’s own traversal-step labels, not canvas edge ids (its edges are `E0001`…).
Nothing can turn one into the other without inventing the evidence a check cites, so the unresolvable
ones are **dropped and each named**: an empty `sanity_checks` is a state the Query tab already handles
by abstaining, and an abstention is honest where a written verdict lighting up the wrong sub-graph is not.

**The report section does not draw CAPEX yet, and that is gated rather than refused.**
CAPEX’s reports are scoped `sc_author_all` over a `projects` spine and drawn by nineteen block
renderers (`figRow`, `bar`, `heatmap`, `chain`, …) that live in `src/report/` — a standalone Vite app,
not vendored in. **"This document is broken" and "this server does not draw this format" are different
facts, and only the first is a reason to refuse a boot**: `validateDb` refuses a report whose
references are broken because every one of them *renders*, but a scope filter this server lacks is not
a defect in a document its package generated. Refusing the boot over it took the whole dataset down —
Sources, Catalog, the graph, the lens — because one section cannot draw one format.

So `reportDrawable` decides **per definition**, `reportFormatGap` names the rest, and
`datasetGapSentence` is written once so six surfaces cannot word one fact six ways. Per definition
rather than per document, because gating the section cost `both` its reports: the merged list is
EPA’s five drawable definitions plus CAPEX’s three undrawable ones, and an early return took the five
with them. The drawable ones are served, the page gates only when **nothing** is drawable, and it
states the sentence beside the list otherwise — a section three cards shorter reads as a package that
shipped three fewer, which is what `governance.ungoverned` exists to prevent one layer up.

**And the gate is stated above the publish gate, which is the order that matters.** Publishing a
graph would not make an undrawable format drawable, so `NoPublishedGraph` first would send a reader to
Graph Studio to do something that changes nothing. Audit & Governance has its own precondition and its
own sentence (`registerGap`): every basis and every resolution on that page reads
`reports.data.generators`, and CAPEX’s rosters are projects, contracts and change orders. Two helpers
rather than one, because they gate two surfaces on two different facts and one function for both would
explain the wrong page with the wrong sentence.
...
**`both` merges two real documents again**, so the machinery this section describes is exercised
rather than merely intact. Its `MERGE_PLAN` entry for `reports.data` now names EPA’s four spines and
CAPEX’s six — `projects` keyed by `n`, `contracts` and `changeOrders` by `id`, and `regions`,
`business_units`, `categories` as `{ union: null }` because they are arrays of **strings**, facet
vocabularies rather than rows. A roster with no rule drops out of the merged document, which
`validateDb` never sees because it validates the two real documents rather than the view built from
them. `_meta` and `_provenance` are the package’s own generator provenance, read by nothing, and
`primary` on the reasoning that a merged document has **no single producer** — so `both` carries none
and claims none. A key with no rule stops the boot, which is that check working.
...
**It is one of three stylesheets exempt from the `--sp-*` spacing rule**, and all three are
vendored. 173 spacing declarations on a 2px rhythm here, the graph viewer's own in
`src/graph-viewer/styles.css`, and the CAPEX report layer's in `src/capex-report/capex-report.css` —
none expressible on a 4px scale without redrawing somebody else's design, all carried over unchanged.
The exemption is a **named list**, and `check-docs` asserts it holds nothing but those three vendored
paths: nothing authored in this repo joins it, and "vendored" cannot come to mean "inconvenient to
convert".

**`src/report/` is excluded from the stylesheet sweep rather than exempted, and those are different
facts.** It is the standalone CAPEX report app, checked in as the source
`scripts/scope-capex-css.mjs` vendors from: nothing in this app imports it, and it carries its own
`package.json`, router and entry point. Sweeping it would flag a sheet the app never loads; exempting
it would put that sheet on a list whose whole job is to name the vendored sheets the app *does* load.

#### Two report formats, and the payload says which

**A report is either computed here or resolved by its own package, and only one of those is
`reportView`'s.** EPA's five are questions asked of rosters this app holds, so every figure is derived
per request. CAPEX's three are not: they are a resolver's output — `reports.resolved[reportId]` in the
document, every number already a `display` or `exact` string — and re-deriving them here would be a
second implementation of the aggregation rules that agrees with the first exactly until one is edited.

`GET /reports/:id` therefore answers with **`report`** or **`resolved`**, never both, and the client
forks on that discriminator rather than sniffing the shape — the same honesty `variant: written |
generated` keeps on a built report. `reportComputable` asks "can this server compute it?",
`resolvedRun` asks "did its package answer it?", and `reportDrawable` is the union; the format gap now
names only definitions with **neither**.

**A resolved run states its own row counts, and they are read.** `reportCounts` returns
`rowsScoped`/`rowsTotal` from the run rather than filtering a roster — a resolved report's scope names
a filter this server does not implement, so `reportRows` threw
`REPORT_SCOPES[report.scope] is not a function` and the whole section 400d. Same rule its figures
follow, applied to its row count.

**`validateDb` skips a resolved definition's references, per report.** Every one of those checks
describes something `reportView` would do — apply a scope, label a bar from a spine column, chart a
measure the rows carry — so `sc_author_all` over a `projects` spine is not a defect, it is simply not
this server's business. What is still checked for both kinds: the spine resolves to a non-empty
roster. `npm run adopt:dataset` additionally refuses a definition with no run **and** a run with no
definition, so neither kind can be half-present.

**The renderers are vendored, in `src/capex-report/`.** Nineteen block renderers, the primitives, the
filter and trust bars, the state provider and a 2,322-line stylesheet, carried over from
`src/report/` the way `src/reports/` and `src/graph-viewer/` were — not reimplemented. Three files
here are this repo's: `CapexReport.tsx` (the seam), `host.tsx` and `specs.js`. Four changes were made
to the vendored code and no others: imports repointed for the flattened layout; `Calendar.jsx` reads
the served specs instead of importing the 1.5 MB fixture; `Calendar.jsx` opens a report through the
host instead of a `react-router` `<Link>`; and the stylesheet is scoped under `.cw-capex-report`.

**That link mattered.** The standalone app routes `/reports/:slug`; this one has a single `/reports`
address and keeps `openId` in a store, so the drill from a filing month into Project 360 navigated to
nothing — and crashed `renderToString` for want of a router. `host.tsx` injects the act instead, going
through the store's own `open()`, which is the same path the Library's button takes. **The coordinate
travels and is not applied**: one resolved run per report is carried, so `ReportView`'s `seedMismatch`
notice says *"this run is resolved at X, not Y"* rather than answering under the wrong project.

**The fidelity check is the upstream app's own.** Every `display` (or `exact`) string in a run must
appear in the rendered HTML — the assertion that catches a renderer reading a field the resolver
renamed, which is how a whole block once shipped as a masking notice for a masking that was not
happening. It renders, so nothing fails. Two exclusion lists are copied from upstream rather than
re-derived: `COLLAPSED` (six keys behind an interaction) and `NOT_DRAWN` (three figures never printed,
each with its reason). Omitting either reported correctly-drawn figures as missing — 66 and 62 of
them. All three reports pass at 414/414, 72/72 and 156/156.
...
- **Facets are per spine**, and the frame validator asks the same `reportFacetsFor` the facets
  ...
  declared (`slice_default` + `fields.filterable`); the other three are derived from whatever
  distinguishes their rows — a facility's `role`, a quarter's `year`, a trace's `flag`
  (three columns, one control).

  **Two of those three are not columns, and a facet that is not a column has to filter by a
  test.** `reportFrameRows` otherwise compares `String(row[key])` against the selected values,
  which selects *nothing at all* where no such column exists — the quarters roster carries
  `quarter` (`"2023 Q1"`) and nothing named `year`. That is not an error, it is a report with no
  rows, and three blocks then read their column headers off `rows[0]`: `Object.keys(undefined)`,
  which reaches the reader as **"Could not open that report"** and names neither the report nor
  the chip that emptied it. So `REPORT_DERIVED_FACETS` declares the test **once** and both sides
  read it — `reportFrameRows` to select the rows and `reportFacetsFor` to count them — because a
  chip whose count and whose filter disagree is the same two-answers failure this section refuses
  everywhere else. `flag` is the third, and stays its own branch: it is one control over three
  columns rather than one derived value.

  **And a block takes its columns from the roster, never from the rows in view.** A frame may
  legitimately select nothing, and a table with no rows still has headers — `reportSpineKeys`
  falls back to the roster so an empty selection renders an empty table rather than a 500.
