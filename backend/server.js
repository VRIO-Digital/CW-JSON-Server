...
const routes = [
  ...
  /*
   * **Accepts every undecided stored relation at once — one request, one commit.**
   *
   * Asked for as a bulk act rather than a sequence: *"when the user clicks Accept all it should bulk
   * accept"*. The relations dialog used to run the row's own write down the list, which is 59
   * requests, 59 validations and 59 document commits for one decision — and a refusal partway
   * through left half the list accepted, which is the one outcome a reader cannot tell from a bug.
   * This is the arrangement the multi-dictionary schema upload already has: **resolve everything
   * before writing anything, and land it in a single `commitDb`**, so a refusal leaves the document
   * exactly as it was.
   *
   * **It is not a second way to write a declaration**, which is what would have made a route here
   * wrong. `POST /data-model/entities` still owns declaring, editing and deleting — every field of
   * every relationship is hers. This one writes `confirmed_by` on rows that already exist and
   * touches nothing else, which is why it can be a route of its own without becoming a second
   * answer to what a declaration is.
   *
   * Four rules, each guarding something that fails quietly:
   *
   *  - **It only fills a `confirmed_by` that is null.** A row somebody else accepted keeps their
   *    name — the `studioPublishedBy` lesson, where a record about an *act* that was only ever set
   *    and never rewritten went on crediting whoever went first. Here the hazard is the mirror:
   *    rewriting would take an acceptance off somebody who made it.
   *  - **Scope is both ends of the relationship**, so what this accepts is exactly what the dialog
   *    listed: `declaredRelationshipsFrom` drops a declaration whose target is not a table in
   *    scope, and a route accepting rows the reader could not see would make the button's count a
   *    number about something else.
   *  - **Absent `table_keys` means every table in this dataset; `[]` is refused.** "Accept nothing"
   *    and "accept everything" are opposite requests, and a route that could not tell them apart
   *    would answer one with the other — the rule `POST …/profile-mail-documents` keeps for its own
   *    `objects`.
   *  - **Nothing undecided is a refusal naming what it found**, not a 200 reporting zero: a run
   *    that committed nothing and said "ok" reads as an act that happened.
   *
   * **Deliberately unpaced.** The suggesters are held because an analysis returning in 2ms teaches
   * that it is free; this is a write, like every other declaration write here and like
   * `POST /reports/build`, and its result is 59 badges changing on screen.
   */
  {
    method: 'POST',
    match: (p) => p === '/data-model/relationships/accept',
    handle: async (req, res, { query }) => {
      ...
      if (!as || !as.includes('@')) {
        return send(res, 400, {
          error:
            'as=<email> is required — the identity is held in the browser, so this route has to be told who is accepting. Accepting on behalf of nobody would record an empty name.',
        })
      }
      const who = as.trim()

      const scope = body.table_keys
      if (scope !== undefined) {
        if (!Array.isArray(scope) || scope.some((k) => typeof k !== 'string')) {
          return send(res, 400, {
            error: 'table_keys must be an array of "<dataset>.<table>" strings, or absent for every table in this dataset',
          })
        }
        if (scope.length === 0) {
          return send(res, 400, {
            error:
              'table_keys is empty — accepting nothing and accepting everything are opposite requests. Omit the field to accept every stored relation in this dataset.',
          })
        }
      }
      const inScope = scope === undefined ? null : new Set(scope)
      const covered = (tableKey) => inScope === null || inScope.has(tableKey)

      /*
       * Resolved first, across every entity, so the counts are known before anything is written —
       * and so `already` can say why a run accepted fewer rows than the list holds.
       */
      let accepted = 0
      let already = 0
      let total = 0
      const entities = db.data_model.entities.map((entity) => {
        ...
        const relationships = (entity.relationships ?? []).map((r) => {
          /* Both ends, or this accepts a row the dialog never showed. */
          if (!covered(r.target_table_key)) return r
          total += 1
          if (typeof r.confirmed_by === 'string' && r.confirmed_by.includes('@')) {
            already += 1
            return r
          }
          accepted += 1
          touched = true
          return { ...r, confirmed_by: who }
        })
        if (!touched) return entity
        return { ...entity, relationships, updated_at: new Date().toISOString() }
      })
      ...
      if (accepted === 0) {
        return send(res, 400, {
          error:
            total === 0
              ? 'there are no stored relations in scope to accept — declare one, or accept a suggestion first'
              : `all ${total} stored relation(s) in scope have already been accepted — nothing was changed`,
        })
      }

      /* One commit for the whole run: all of them or none, so a refusal cannot leave half the list
         accepted with nothing on screen saying which half. */
      await commitDb({ ...db, data_model: { ...db.data_model, entities } })

      send(res, 200, { accepted, already, total, accepted_by: who })
    },
  },
