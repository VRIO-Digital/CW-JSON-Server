...
const sourcePath = join(root, 'mock-server', 'db.json')
let source
try {
  source = JSON.parse(await readFile(sourcePath, 'utf8'))
} catch (error) {
  die(
    `could not read ${PRIMARY}'s document at mock-server/db.json — ${error.message}\n` +
...
console.log(`seed-dataset: wrote mock-server/db.${target}.json`)
