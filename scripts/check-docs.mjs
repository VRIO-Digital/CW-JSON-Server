...
/**
 * One function body, sliced by its signature and matching braces.
 *
 * A whole-file search cannot express "no *block* reads its columns off rows[0]" while the same
 * expression appears elsewhere, correctly guarded, in `validateDb` — the claim would fail against
 * code that is right. Braces are counted rather than matched with a regex: a non-greedy tail to the
 * first `
}` stops at whichever nested brace reaches column 0 first, and a greedy one runs to the
 * end of the file. Returns null when the signature is absent, so a claim can say so rather than
 * assert over an empty string — the vacuous-assertion trap this file has fallen into before.
 */
...
/*
 * A secondary dataset's document, when this checkout has one.
 *
 * **Optional on purpose, and that is not a fallback that hides a fault.** `DATASETS` is what decides
 * whether a dataset exists — a name there with no document stops the *boot*, which is the check that
 * matters. Here the document is only ever an extra source of rows to check a rule against, so a
 * checkout without it loses coverage rather than gaining a false pass: every claim that reads it
 * filters it out and still asserts over the primary. `readJson` yields `{}` for a missing file, so
 * `here` is what tells "absent" from "empty".
 */
const capexDocFile = readJson('mock-server/db.CAPEX.json')
const capexDoc = capexDocFile.here ? capexDocFile.value : null
 ...
 */
/*
 * Every stylesheet the **app** carries.
 *
 * `src/report/` is skipped: it is the standalone CAPEX report app, checked in as the source
 * `scripts/scope-capex-css.mjs` vendors from, and nothing in this app imports it — it has its own
 * `package.json`, its own router and its own entry point. Sweeping it would flag a sheet the app never
 * loads, and *exempting* it would put that sheet on a list whose whole job is to name the vendored
 * sheets this app does load. Excluded, not exempted, and the two are different facts.
 */
const VENDOR_SOURCE_DIRS = ['src/report']
const cssFiles = (function walkCss(dir) {
  if (VENDOR_SOURCE_DIRS.includes(dir)) return []
 ...
 *
 * The exemptions are **narrowed rather than taken on trust**: each file must stay scoped under its
 * own class (checked below), and this list must contain nothing but the three vendored sheets — so
 * nothing authored in this repo can quietly join it, and none of them can quietly go global.
 *
 * The third is `src/capex-report/capex-report.css`, generated by `scripts/scope-capex-css.mjs` from
 * the CAPEX report layer's own stylesheet with 1,455 rule bodies asserted identical to the source.
 * Converting its rhythm would mean redrawing somebody else's design, which is the ground all three
 * stand on.
 *
 * `src/report/` — the standalone app the third was vendored *from* — is **excluded from the sweep**
 * rather than exempted, and the difference matters: nothing in the app imports it, it carries its own
 * `package.json` and router, and it is checked in as vendor source. Exempting its sheet would put a
 * file the app never loads onto a list that exists to name the sheets it does.
 */
const SPACING_EXEMPT = [
  'src/reports/reports-prototype.css',
  'src/graph-viewer/styles.css',
  'src/capex-report/capex-report.css',
]
expect(
  'the spacing rule is exempt for the three vendored stylesheets and nothing else',
  SPACING_EXEMPT.length === 3 &&
    SPACING_EXEMPT.every((f) => cssFiles.includes(f)) &&
    /* Vendored means it sits in a directory this repo did not author the design of. All three are
       named, so "vendored" cannot come to mean "inconvenient to convert". */
    SPACING_EXEMPT.every(
      (f) =>
        f.startsWith('src/reports/') ||
        f.startsWith('src/graph-viewer/') ||
        f.startsWith('src/capex-report/'),
    ),
...
expect(
  ...
  /*
   * **The shape, not the count — which this claim said and did not do.** It pinned
   * `DATASETS = ['EPA']` literally, so adopting CAPEX's document made it stale while every fact in
   * its own comment was still true. That is the drift already on the pitfall list twice: a claim
   * keyed to the spelling fails on a change the fact survives, and a guard that cries wolf is how a
   * real red claim comes to be ignored.
   *
   * What has to hold: the list is declared and non-empty, the primary is one of its members, and it
   * is **first** — that ordering is load-bearing, because `both` resolves every single-valued key to
   * the primary and `mergeDocs` reads this order to decide which that is.
   */
  (() => {
    const decl = datasets.split('export const DATASETS = [')[1] ?? ''
    const names = [...(decl.split(']')[0] ?? '').matchAll(/'([^']+)'/g)].map((m) => m[1])
    const primary =
      datasets.split("export const PRIMARY = '")[1]?.split("'")[0] ?? null
    return names.length > 0 && primary !== null && names[0] === primary
  })() &&
...
/*
 * ---------------- a second dataset, adopted rather than seeded ----------------
 *
 * CAPEX's document is its own package's — Northline Water Group, generated against this repo's
 * schema — so it is **adopted**, not seeded. `npm run seed:dataset` writes the primary's structure
 * with its rows removed, which is right for a dataset that has none and would throw this one away.
 *
 * Three decisions in that script are load-bearing, and each one is the difference between a real
 * second dataset and EPA's figures under another name:
 *
 * - the **tenant's directory travels as one unit**. `settings` references `auth_roles` and
 *   `validateDb` enforces it, so carrying half the pair left every user naming a role the document
 *   did not have. The package generated its own four personas; they are set aside and said so.
 * - a missing **collection** is a refusal, never filled from the primary.
 * - it is *not* "every `primary` key comes from the primary": `whatif` is `primary` too and CAPEX
 *   has its own facility and its own candidate loads.
 */
const adopt = read('scripts/adopt-dataset.mjs')
expect(
  'a package-generated dataset is adopted, and only the tenant directory is carried',
  /const TENANT_IDENTITY = \['auth_roles', 'settings', 'google_account'\]/.test(adopt) &&
    /* Carried whether or not the package brought its own, or the pair drifts apart. */
    /adopted\[key\] = tenant\[key\]/.test(adopt) &&
    /* And a substitution is named rather than silent. */
    /setAside\.push\(key\)/.test(adopt) &&
    /* A collection is refused, so no dataset can serve the primary's rows under its own name. */
    /holds this dataset's own rows — the package generator has to produce it/.test(adopt) &&
    /* Every spine a report names has to resolve before the file is written. */
    /is asked over the/.test(adopt) &&
    /* It writes a file only, like every seed here — it has to run with no credentials. */
    !/db:push|PutObject|s3:/.test(codeOnly(adopt).replace(/npm run db:push/g, "")),
  'seeding over a package’s own rows throws the dataset away; borrowing the primary’s renames them',
)

/*
 * **"This document is broken" and "this server does not draw this format" are different facts.**
 *
 * `validateDb` refuses a report whose references are broken, because every one of them *renders*: a
 * missing spine gives authored tiles above an empty table. A scope this server has no filter for is
 * not that — CAPEX's reports are scoped `sc_author_all` over a `projects` spine, drawn by nineteen
 * block renderers that have not been vendored in, and the document is exactly what its package
 * generated. Refusing the boot over it took the whole dataset down — Sources, Catalog, the graph,
 * the lens — because one section cannot draw one format.
 *
 * So the section gates instead, and the two halves have to hold together: while the gap is
 * non-empty **no report is served and every report route refuses**, so nothing renders
 * half-computed; and the sentence is the **server’s**, on the payload, so the page cannot describe
 * a gap that had already been closed.
 */
/*
 * **A node that states no confidence is not given one, and that is two different bugs.**
 *
 * `confidence` is nullable on a canvas node, and a whole graph can state none: every one of CAPEX's
 * 442 does. EPA's 189 all state one — 178 of them exactly `1` — which is why neither surfaced until
 * a second dataset arrived.
 *
 * 1. `studioCanvas` read `n.confidence.toFixed(2)` for a proposed node. A hard `TypeError` inside a
 *    **build-step timer**, so it killed the process rather than failing one request, and 14 CAPEX
 *    nodes carry a review item — a build crashed unconditionally. `proposed · 0.00` would have been
 *    the worse fix: a stated certainty of zero is a claim the graph never made.
 * 2. `confidenceOf` fell back to `?? 1` — *completely certain* — so an answer over that graph read
 *    `Confidence 1.00 — the weakest entity on the route`. Worse than the crash: a fabricated figure
 *    on the one line whose whole job is to say how much to trust the answer. A single unstated node
 *    makes the weakest link **unknowable**, so it is null rather than 1 and rather than the minimum
 *    of the rest, which would claim a floor the unstated node could sit below.
 * 3. And `n.confidence < 0.85` counted an unstated one as low, because `null < 0.85` is true — a
 *    chip reading 442 low-confidence nodes on a graph that states none.
 *
 * Same rule in all three, and it is the one the What-if baselines already keep: an absence has no
 * number, because 0 (or 1, or "low") would be a claim. `confidence` is already `number | null` on the
 * answer envelope and on every citation, so the honest answer needed no new field to carry it.
 */
/*
 * **A dataset's own column class folds onto the platform's, and the fold is checked at boot.**
 *
 * `client.ts` declares the classes it can render — nine for a column, six for a document entity — and
 * a class outside that union does not render *anything*: `validate` refuses the whole payload, so the
 * Data Catalog shows `class should be one of … got "organisation"` and advises restarting the mock
 * server, which is not the problem. CAPEX ships sixteen classes of its own, so the section was
 * unreadable for that dataset while profiling itself worked perfectly.
 *
 * Folded rather than admitted, for the reason `FACET_FOR_TYPE` folds a `doc_type` onto four buckets:
 * the facet chips count the platform classes, so a sixteenth renders as a chip nothing counts — and a
 * facet stuck at 0 reads as "none of these in this corpus" rather than as a broken map. Before the
 * fold CAPEX's Measures chip read 0 against twenty measure columns.
 *
 * **The package's own class is kept**, on `class_detail`, and is what the chip prints — so the reader
 * sees `period_accumulation` while Measures counts it. Nothing is discarded to make the union hold,
 * and the tint stays keyed to the folded class because there are nine tints rather than sixteen.
 *
 * Refused at **boot** when nothing folds a class, naming it, for the reason a canvas edge with an
 * unresolved endpoint is: `classFor` returns null and the call sites fall back to `text`, which would
 * classify a measure as prose — a wrong answer rather than a missing one.
 */
expect(
  'a dataset\u2019s own column class folds onto one this app renders, or the boot refuses',
  /* Both vocabularies, because their unions differ: a column may be `flag`, an entity may not. */
  /const COLUMN_CLASSES = \[/.test(server) &&
    /const ENTITY_CLASSES = \['identifier', 'dimension', 'entity', 'measure', 'date', 'text'\]/.test(
      server,
    ) &&
    /const PLATFORM_CLASS_FOR = \{/.test(server) &&
    /const classFor = \(klass, allowed = COLUMN_CLASSES\) => \{/.test(server) &&
    /* The fold is applied on every path a class reaches the client by: the profiled dictionary, the
       synthesised columns, and the synthesised document entities. */
    (server.match(/class: classFor\(/g) ?? []).length === 3 &&
    (server.match(/class_detail: classDetailFor\(/g) ?? []).length === 3 &&
    /* The entity path folds against the narrower union, or a `flag` entity is refused downstream. */
    /classFor\(v\.class, ENTITY_CLASSES\)/.test(server) &&
    /* An unmapped class stops the boot rather than the render. */
    /which nothing folds onto a class this app/.test(server) &&
    /* And the client carries the finer class rather than the panel inventing a second map. */
    (client.match(/class_detail: nullable\(str\)/g) ?? []).length === 2 &&
    /\{col\.class_detail \?\? col\.class\}/.test(codeOnly(read('src/components/ProfiledColumnsPanel.tsx'))) &&
    /* Every class either dataset actually ships has to fold — asserted against the documents, so a
       seventeenth class in a package fails here rather than in a browser. */
    (() => {
      const folds = server.match(/const PLATFORM_CLASS_FOR = \{([\s\S]*?)\n\}/)
      if (!folds) return false
      const mapped = new Set([...folds[1].matchAll(/^\s*(\w+):/gm)].map((m) => m[1]))
      const platform = new Set(['identifier', 'dimension', 'entity', 'measure', 'date', 'address', 'geo', 'flag', 'text'])
      const unmapped = []
      for (const doc of [db, capexDoc].filter(Boolean)) {
        for (const key of ['column_vocabulary', 'document_vocabulary']) {
          for (const v of doc[key] ?? []) {
            if (!platform.has(v.class) && !mapped.has(v.class)) unmapped.push(v.class)
          }
        }
        for (const profiled of Object.values(doc.column_profiles ?? {})) {
          for (const c of profiled ?? []) {
            if (!platform.has(c.class) && !mapped.has(c.class)) unmapped.push(c.class)
          }
        }
      }
      return unmapped.length === 0
    })(),
  'a class outside the client\u2019s union does not render short \u2014 it refuses the whole payload',
)

/*
 * **An uncatalogued row count is unknown, and a statistic derived from it is unknown too.**
 *
 * CAPEX's tables state the absence in the document itself (`rows: null`, `rows_basis: "catalogued only
 * -- the package does not hold a row count for this table"`). `?? 0` reported every table as **empty**,
 * which is the one reading that makes profiling look like it did nothing — and it was the reported
 * symptom. Worse, `synthesiseColumns` derives `distinct` from the row count for four classes, and a cap
 * of `Math.min(distinct, Math.max(tableRows, 1))` turned even the resolved ones into 0, because
 * `Math.min(null, 1)` is 0: **every column read "1 distinct value"**, then "0".
 *
 * Third instance of the one rule in this session, after the two confidence bugs: an absence has no
 * number. The cap expresses a fact about the rows, so with no rows catalogued there is no fact.
 */
expect(
  'an uncatalogued row count is served as unknown, and so is anything derived from it',
  /const rows = meta\?\.rows \?\? null/.test(server) &&
    /else if \(typeof tableRows !== 'number'\) distinct = null/.test(server) &&
    /* The cap applies only where the count is known. */
    /typeof tableRows === 'number' && distinct !== null/.test(server) &&
    /* Nullable end to end, or the honest answer is refused at the boundary. */
    /rows: nullable\(num\)/.test(client) &&
    /distinct: nullable\(num\)/.test(client) &&
    /rows: number \| null/.test(client) &&
    /distinct: number \| null/.test(client) &&
    /* And the panel says so rather than printing a figure it does not have. */
    (() => {
      const panel = codeOnly(read('src/components/ProfiledColumnsPanel.tsx'))
      return /row count not catalogued/.test(panel) && /v === null \? \(/.test(panel)
    })(),
  'a table reading 0 rows reads as empty, which is what made profiling look broken',
)

expect(
  'an unstated confidence is reported as unstated, never as certain or as low',
  /* The crash: a proposed node with no stated confidence says only that it is proposed. */
  /\? \x27proposed\x27/.test(server) &&
    /* Unknown rather than 1, and one helper so the answer and its citations cannot disagree about
       what the weakest link was. The positive form is the assertion: a null fallback is what makes
       an unstated confidence unstated. */
    /const confidenceOf = \(nodeId\) => nodeOf\(nodeId\)\?\.confidence \?\? null/.test(server) &&
    /const weakest = \(ids\) => \{/.test(server) &&
    /const confidence = weakest\(walk\.path\)/.test(server) &&
    /confidence: weakest\(\[e\.from, e\.to\]\)/.test(server) &&
    /* And the reasoning step says which it is rather than printing a figure it does not have. */
    /Confidence is not stated/.test(server) &&
    /* The facet counts what is known to be low, not what is merely unstated. */
    /typeof n\.confidence === 'number' && n\.confidence < 0\.85/.test(server) &&
    /* The envelope can carry it, which is what made the honest answer expressible. */
    /confidence: number \| null/.test(client),
  'a fabricated certainty on the line that reports certainty is worse than the crash it replaced',
)

const capexSeam = read('src/capex-report/CapexReport.tsx')
const capexHost = read('src/capex-report/host.tsx')
const capexCalendar = read('src/capex-report/blocks/Calendar.jsx')
const capexCss = read('src/capex-report/capex-report.css')
const capexPane = codeOnly(read('src/components/report/PublishedReportPane.tsx'))

/*
 * **A report is computed here or resolved by its own package, and the payload says which.**
 *
 * `reportView` derives every figure per request, which is right for EPA's five — questions asked of
 * rosters this app holds. CAPEX's three are a resolver's output, every number already a `display`
 * string, and recomputing them would be a second implementation of the aggregation rules that agrees
 * with the first exactly until one is edited. So the server serves one kind or the other and **names**
 * which, and the client forks on that rather than sniffing the shape — the same honesty
 * `variant: written | generated` keeps on a built report.
 */
expect(
  'a report is computed here or resolved by its package, and never both',
  /const resolvedRunIn = \(doc, reportId\) =>/.test(server) &&
    /const reportComputable = \(report\) =>/.test(server) &&
    /const reportDrawableIn = \(doc, report\) =>/.test(server) &&
    /* Exactly one kind on the payload: the resolved branch returns `report: null` beside it. */
    /resolved !== null && !reportComputable\(report\)/.test(server) &&
    /* A resolved run states its own row counts rather than a roster being filtered by a scope this
       server does not implement — which threw `REPORT_SCOPES[report.scope] is not a function`. */
    /const reportCounts = \(report\) => \{/.test(server) &&
    !/row_count: reportRows\(r\)\.length/.test(codeOnly(server)) &&
    /* And the validator skips a resolved definition's references, per report. */
    /if \(resolvedRunIn\(candidate, r\.report_id\) !== null\) continue/.test(server) &&
    /* Typed, and the pane forks on the discriminator rather than on the shape. */
    /resolved: nullable\(shape\(\{ id: str, name: str, blocks: arrayOf\(anyObject\) \}\)\)/.test(client) &&
    /\) : resolved \? \(/.test(capexPane) &&
    /<CapexReport/.test(capexPane),
  'recomputing a resolved figure is a second implementation of somebody else\u2019s aggregation rules',
)

/*
 * **The CAPEX renderers are vendored, and the changes made on the way in are countable.**
 *
 * Same discipline as `src/reports/` and `src/graph-viewer/`: the folder is carried over, three files
 * beside it are this repo's, and every change to the vendored code is named. The two that matter are
 * the ones that would otherwise put a fixture in the bundle or a dead control on the page.
 */
/*
 * **The CAPEX report renders light, because this app is light and the sheet defaults to dark.**
 *
 * The vendored sheet declares its palette as `:root, [data-theme="dark"]{…}` with
 * `[data-theme="light"]{…}` below — the standalone app shipped `data-theme="light"` on `<html>` and its
 * `Shell` toggled it. `Shell` was dropped on the way in (this app draws its own chrome), so nothing set
 * the attribute and the report rendered **dark inside a light page**.
 *
 * **And the scoping made it unfixable, which is the subtler half.** A bare `[data-theme="light"]`
 * qualified the *page*; prefixed as a descendant it became `.cw-capex-report [data-theme="light"]`,
 * which can only match an element inside the report carrying that attribute — nothing does. So the
 * light palette could not win however the wrapper was marked. A leading attribute selector attaches to
 * the wrapper; `[hidden] .foo` is a real descendant rule and is still prefixed normally.
 *
 * Same class of problem as the graph viewer being vendored dark, with the opposite remedy: there the
 * palette had to be rebuilt, here it already existed and only needed asking for.
 */
expect(
  'the CAPEX report renders in the light palette the sheet already carries',
  (() => {
    const seam = read('src/capex-report/CapexReport.tsx')
    const css = read('src/capex-report/capex-report.css')
    const scoper = read('scripts/scope-capex-css.mjs')
    return (
      /* Over **code**: this file’s own comment explains the attribute, so a whole-file search passed
         with the JSX attribute deleted. Seventh time this trap has fired in one session — `codeOnly`
         belongs on every whole-file claim, and the token has to be the rendered form. */
      codeOnly(seam).includes("className=\"cw-capex-report\" data-theme=\"light\"") &&
      /* Qualifying the wrapper, not a descendant of it. */
      css.includes('.cw-capex-report[data-theme="light"]{') &&
      !css.includes('.cw-capex-report [data-theme="light"]') &&
      /* Able to win: later in the file than the default block. */
      css.indexOf('.cw-capex-report[data-theme="light"]{') >
        css.indexOf('.cw-capex-report, .cw-capex-report[data-theme="dark"]{') &&
      /* And the block is not empty — the light ground is really in it. */
      /* Sliced to the block rather than matched with a regex: a character class for
         `[data-theme="light"]` has to survive several layers of escaping to land intact, and it did
         not — it arrived as a character class and matched nothing, which is a vacuous conjunct. */
      css
        .slice(css.indexOf(".cw-capex-report[data-theme=\"light\"]{"))
        .split("}")[0]
        .includes("--bg1:#ffffff") &&
      /* **Two** such returns: the `body|html|:root` rule and the leading-attribute rule. A bare
         `includes` passed with the attribute rule deleted, because the first one’s
         `SCOPE + sel.replace(…)` contains the same substring — so this counts them. Split rather than
         a regex, to keep it free of escapes that have been eaten three times in this file. */
      scoper.split("return SCOPE + sel").length === 3
    )
  })(),
  'a descendant selector cannot carry a palette meant for the page it was written for',
)

expect(
  'the CAPEX renderers are vendored, scoped, and fed by the server rather than a bundled fixture',
  /* The seam mounts the tree inside the class the stylesheet is scoped under — without it the sheet
     does nothing, and unscoped it would reset every antd component in the app. */
  /className="cw-capex-report"/.test(capexSeam) &&
    /\.cw-capex-report/.test(capexCss) &&
    /* Every selector scoped: a bare `*` or `body` rule here resets the whole app, silently. */
    (() => {
      const scanned = capexCss.replace(/\/\*[\s\S]*?\*\//g, '')
      const preludes = scanned.match(/(?:^|\})[^{}@]*\{/g) ?? []
      const unscoped = preludes.filter(
        (p) => !p.includes('.cw-capex-report') && !/(?:^|\})\s*(?:from|to|\d+(?:\.\d+)?%)\s*\{/.test(p),
      )
      return preludes.length > 1000 && unscoped.length === 0
    })() &&
    /* The specs come off the wire, not out of a 1.5MB import compiled into the bundle. */
    /import \{ reportById \} from '\.\.\/specs\.js'/.test(capexCalendar) &&
    !/data\/db\.json/.test(capexCalendar) &&
    /* And no renderer imports the vendor source directory, which would pull the whole app back in. */
    (() => {
      const files = readdirSync(join(root, 'src/capex-report'), { withFileTypes: true })
        .filter((e) => e.isFile() && /\.(jsx|js|tsx|ts)$/.test(e.name))
        .map((e) => codeOnly(read(`src/capex-report/${e.name}`)))
      /* Over code again: several of these files' own comments say "vendored from `src/report/`". */
      return (
        files.length > 5 &&
        files.every((f) => !/from '[^']*\.\.\/report\//.test(f) && !/require\('[^']*report\//.test(f))
      )
    })(),
  'a fixture compiled into the bundle is the one thing on screen the bucket cannot change',
)

/*
 * **The calendar's drill-through goes through the host, not a URL this app does not have.**
 *
 * The standalone app routes `/reports/:slug`; this one has a single `/reports` address and keeps
 * `openId` in a store. The vendored `<Link to="/reports/project-360?project=…">` therefore navigated to
 * nothing — a control worse than a disabled one — and crashed `renderToString` outright for want of a
 * router, which is how it was found.
 *
 * **The coordinate travels and is not applied.** One resolved run per report is carried, so opening
 * Project 360 at another project cannot re-resolve it: `ReportView`'s `seedMismatch` names the run's own
 * coordinate instead of answering under a name the reader did not click.
 */
/*
 * **Open and Edit are different acts, so they are gated separately — and one flag gating both is how a
 * report became unopenable from the only list that offers it.**
 *
 * `GovernedCard` computed `openable = !!starterForTag(row.reportTag, row.reportId)` and hid *both*
 * buttons on it. Correct while every governed row resolved to one of the prototype's five starters: the
 * definitions and the starters were the same five reports out of the same file.
 *
 * It fails for a dataset whose reports the host renders. CAPEX's rows carry `R1`–`R3`, and the starters
 * are the **primary's** — `reports_prototype` is `primary` in `MERGE_PLAN`, so a secondary dataset
 * carries the tenant's sample data rather than its own. Nothing matched, so **no Open button was drawn**
 * while `GET /reports/:id` served the report perfectly well. Silent, and invisible from the server side:
 * every endpoint answered 200.
 *
 * The split is what the labels always claimed, and `openGoverned` already implemented it — Open returns
 * early through `onOpenPublished` before `fromGoverned` is ever called. Only the card disagreed.
 */
expect(
  'Open and Edit are gated on their own preconditions, not on one flag',
  (() => {
    const card = codeOnly(read('src/reports/panes/GovernedCard.tsx'))
    const pane = codeOnly(read('src/reports/panes/LibraryPane.tsx'))
    const app = codeOnly(read('src/reports/App.tsx'))
    return (
      /* Two names, two questions: can anything render it, and is there a definition to load. */
      /const hasStarter = !!starterForTag\(/.test(card) &&
      /const canOpen = hostRenders \|\| hasStarter/.test(card) &&
      /\{onOpen && canOpen && \(/.test(card) &&
      /\{onEdit && hasStarter && \(/.test(card) &&
      /* The single flag is gone, or the two can silently converge again. */
      !/const openable/.test(card) &&
      /* Threaded from the only place that knows whether a host is there to render. */
      /hostRenders=\{hostRenders\}/.test(pane) &&
      /hostRenders=\{!!onOpenPublished\}/.test(app) &&
      /* And Open still goes to the host rather than loading an authoring definition. */
      /if \(!forEdit && onOpenPublished\) \{/.test(app)
    )
  })(),
  'a governed row whose reports the host renders needs no authoring starter to be opened',
)

expect(
  'the drill-through is the host\u2019s act, and the coordinate it carries is stated rather than applied',
  /* No router in the vendored folder any more — over **code**, because the comment that replaced the
     import names `react-router` in explaining why it is gone. That is the self-documenting-file trap
     this file has now recorded six times: `codeOnly` is for every whole-file absence claim, not only
     the ones where the trap is obvious. */
  !/react-router/.test(codeOnly(capexCalendar)) &&
    /const host = useCapexHost\(\)/.test(capexCalendar) &&
    /host\.openReport\(target\.id, \{ param: drill\.param, value: String\(value\) \}\)/.test(capexCalendar) &&
    /* The seam is a context with a no-op default, so the folder standing alone draws a disabled chip. */
    /export const CapexHostContext = createContext<CapexHost>\(NO_HOST\)/.test(capexHost) &&
    /openReport: \(\) => \{\}/.test(capexHost) &&
    /* The host routes it through the store's own open(), the same path the Library's button takes. */
    /void open\(reportId\)/.test(capexPane) &&
    /* And the mismatch is computed against `activeParams` — the run's own record of what it holds —
       never a param's `selected`, which a spec default can populate without the run being narrowed. */
    /const active = \(v\.activeParams \?\? \{\}\)/.test(capexSeam) &&
    /seedMismatch=\{seedMismatchOf\(view, coordinate\)\}/.test(capexSeam),
  'a link to a route this app does not have is a dead control, and a re-seeded coordinate is a wrong figure',
)

expect(
  'an undrawable report definition is gated per report, not refused at boot',
  /* Declared as pure functions over one document, so both are assertable without a boot. */
  /const reportDrawable = \(report\) =>/.test(server) &&
    /const reportFormatGap = \(doc\) => \{/.test(server) &&
    /const registerGap = \(doc\) => \{/.test(server) &&
    /* Per definition, not per dataset: gating the section would cost `both` EPA's five drawable
       reports because CAPEX's three undrawable ones are merged in beside them. */
    /const drawableReports = \(\) =>/.test(server) &&
    /reports: drawableReports\(\)\.map/.test(server) &&
    /* The validator skips the reference block rather than refusing to boot. */
    /if \(problems\.length === 0 && reportFormatGap\(candidate\)\.length === 0\) \{/.test(server) &&
    /* One sentence builder, so six surfaces cannot word one fact six ways. */
    /const datasetGapSentence = \(what, gaps\) =>/.test(server) &&
    /* The shared frame validator refuses an undrawable definition **before** anything applies its
       scope filter, and after the lookup so the refusal can name the report. */
    (() => {
      const fn = sliceFn(server, 'const reportFrameProblem = (frame) => {')
      if (fn === null) return false
      const drawable = fn.indexOf('if (!reportDrawable(report)) {')
      const lookup = fn.indexOf('db.reports.reports.find')
      const scope = fn.indexOf('db.reports.opts[slot]')
      return drawable > lookup && drawable < scope
    })() &&
    /* No silent cap: the count of undrawable definitions is named, and the page states it beside
       the list as well as instead of one. */
    /of \$\{reports\.length\} definitions are scoped/.test(server) &&
    /format_gap: formatGap,/.test(server) &&
    /* Typed and mapped on the client, and read by the page on both branches. */
    /format_gap: nullable\(str\)/.test(client) &&
    /formatGap: raw\.format_gap \?\? null/.test(client) &&
    (() => {
      const page = codeOnly(read('src/pages/ReportsPage.tsx'))
      return (
        /if \(data\?\.formatGap && data\.reports\.length === 0\) \{/.test(page) &&
        /\{data\?\.formatGap \? \(/.test(page)
      )
    })(),
  'a scope filter this server lacks is not a defect in a document its package generated',
)
...
/*
 * **Per branch, which is what this claim always said and did not check.** It asserted no
 * `<EmptyState>` anywhere in a gated page — right while publication was each page's only
 * precondition, and wrong the moment one of them grew a second. Reports now gates on the selected
 * dataset's report format as well, which is a *different* fact with a different next action, so the
 * blanket absence rule failed against code that keeps the rule exactly.
 *
 * The fact is that **the publication branch renders the shared component** and nothing else does.
 * So a page carrying its own `EmptyState` has to declare which other precondition it is for, and
 * both branches are sliced and checked apart. A page with no second precondition keeps the blanket
 * rule, because there is nothing there an `EmptyState` could legitimately be.
 */
const SECOND_PRECONDITION = { 'src/pages/ReportsPage.tsx': 'if (data?.formatGap && data.reports.length === 0) {' }
const PUBLISH_BRANCH = 'if (data && data.publishedCount === 0) {'
expect(
  'and none of them hand-rolls a second one for the same branch',
  PUBLISH_GATED_PAGES.every((page) => {
    const src = codeOnly(read(page))
    const second = SECOND_PRECONDITION[page]
    if (!second) return !/<EmptyState\b/.test(src)
    /* The second precondition's own branch: its screen, and never the publish gate's. */
    const own = sliceFn(src, second)
    /* The publish gate's branch: the shared component, and never a hand-rolled one. */
    const publish = sliceFn(src, PUBLISH_BRANCH)
    return (
      own !== null &&
      publish !== null &&
      /<EmptyState\b/.test(own) &&
      !/<NoPublishedGraph\b/.test(own) &&
      /<NoPublishedGraph\b/.test(publish) &&
      !/<EmptyState\b/.test(publish) &&
      /* And the second branch is declared *above* the publish gate: publishing a graph would not
         make an undrawable format drawable, so that fix must not be offered first. */
      src.indexOf(second) < src.indexOf(PUBLISH_BRANCH)
    )
  }),
...
/*
 * **A facet the roster carries no column for has to filter by test, and a block's headers come from
 * the roster rather than the rows in view.**
 *
 * Two faults, chained, and neither errored on its own. `reportFacetsFor` derives the quarterly
 * report's `year` chips from `quarter.slice(0, 4)` -- but `reportFrameRows` filtered by equality on
 * `r.year`, a column the quarters roster does not have, so `String(undefined)` matched none of the
 * fourteen rows and the chip selected **nothing**. Not an error: a report with no rows. Then the
 * quarterly block read its column headers off `rows[0]`, and `Object.keys(undefined)` threw
 * `Cannot convert undefined or null to object` -- which reaches the reader as "Could not open that
 * report" and names neither the report nor the filter that emptied it.
 *
 * So both halves are asserted, and the first as the **fact** rather than the spelling: every facet
 * key offered is resolvable against its own spine's roster -- a real column, the `flag` control over
 * three of them, or a declared derived test. A facet added tomorrow whose key is not a column fails
 * here rather than in a 500.
 */
/* `reportBlock` alone — the three blocks that read their columns off the roster live here, and
   `validateDb` elsewhere has a legitimately guarded `Object.keys(rows[0])`. */
const reportBlockBody = sliceFn(server, 'function reportBlock(report, block) {')
const spineFacetKeys = {
  generators: db.reports.slice_default,
  facilities: ['role'],
  quarters: ['year'],
  traces: ['flag'],
}
const derivedFacetBlock = server.match(/const REPORT_DERIVED_FACETS = \{([\s\S]*?)\n\}/)
const derivedFacetKeys = derivedFacetBlock
  ? [...derivedFacetBlock[1].matchAll(/^\s*(\w+): \(row, value\)/gm)].map((m) => m[1])
  : []
const unresolvableFacets = Object.entries(spineFacetKeys).flatMap(([spine, keys]) => {
  const roster = db.reports.data[spine] ?? []
  const columns = new Set(Object.keys(roster[0] ?? {}))
  return keys
    .filter((key) => !columns.has(key) && key !== 'flag' && !derivedFacetKeys.includes(key))
    .map((key) => `${spine}.${key}`)
})

expect(
  'every facet key is a column of its roster, a declared derived test, or the flag control',
  derivedFacetKeys.length > 0 &&
    Object.values(spineFacetKeys).every((keys) => keys.length > 0) &&
    unresolvableFacets.length === 0 &&
    /* `reportFrameRows` consults the derived tests **before** falling through to equality, or a
       declared test is declared and never reached. */
    /const derived = REPORT_DERIVED_FACETS\[key\]/.test(server) &&
    server.indexOf('const derived = REPORT_DERIVED_FACETS[key]') <
      server.indexOf('rows = rows.filter((r) => values.includes(String(r[key])))') &&
    /* And the chip's count runs the same test that will select its rows, so a count cannot describe
       a different set than clicking it returns. */
    /count: rows\.filter\(\(q\) => REPORT_DERIVED_FACETS\.year\(q, value\)\)\.length/.test(server) &&
    /* The second half, asserted **inside `reportBlock`** rather than over the whole file: no block
       reads its columns off the rows in view, so an empty selection renders an empty table with its
       headers instead of throwing. `validateDb` has its own `Object.keys(rows[0])` and guards it with
       a length test, so a file-wide search would fail against correct code. */
    reportBlockBody !== null &&
    !/Object\.keys\(rows\[0\]\)/.test(codeOnly(reportBlockBody)) &&
    /* Every block that has no `cols` of its own takes them from the roster: the scorecard, the
       quarterly profile, the traces, and the quarterly report's second chart. */
    (reportBlockBody.match(/reportSpineKeys\(report\)/g) ?? []).length === 4 &&
    /const reportSpineKeys = \(report\) =>/.test(server) &&
    /db\.reports\.data\[report\.spine\]\[0\] \?\? \{\}/.test(server),
  unresolvableFacets.length > 0
    ? `these facets filter on a column their roster does not have: ${unresolvableFacets.join(', ')}`
    : 'a facet keyed to a column the roster lacks selects nothing, and a block reading rows[0] then throws',
)

/*
 * **A generated report on a spine the summary catalog does not describe states no tiles at all.**
 *
 * `summary_catalog` aggregates the *generator register's* fields, so running it over quarters or
 * traces reads every one off a column that is not there: a year of quarterly volume came back "4
 * inbound generators in view / 0 under EPA enforcement / $0 total penalty exposure", four confident
 * figures about a roster the report is not about. `summary_note` beside them already said the summary
 * was undefined for this spine, so the payload contradicted itself -- and the tiles are the half a
 * reader believes. The note is not the fix; not computing them is.
 */
expect(
  'a re-asked report off the generator register states no summary rather than the register\u2019s',
  /: report\.spine === 'generators'/.test(server) &&
    /* The note still says why, so an absent strip reads as undefined rather than as a failure. */
    /so a re-asked \$\{report\.spine\} report states none/.test(server) &&
    /* And the summary is only ever computed over the rows the frame selected. */
    /reportSummary\(\r?\n\s*report\.summary_keys\.length > 0/.test(server),
  'a tile computed off a roster the report is not about is a wrong figure wearing "computed for this frame"',
)
