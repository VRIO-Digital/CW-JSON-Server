 ...
 *
 * One dependency, `pg`, and it is the only one this folder is allowed. The audit gate
 * fails on any advisory, so a package here has to earn its place — a PostgreSQL driver
 * does, because the alternative to a real database was a 450 KB JSON document doubling
 * as the store. Nothing else gets added on the same argument.
 *
 ...
 * process restarts. Everything else is read from PostgreSQL at startup — from the
 * `contextweave` schema, seeded out of db.json by `npm run db:seed`. **Editing db.json
 * changes nothing until you re-seed.**
 *
...
import { readJsonDb } from './db/read-json.mjs'

const here = dirname(fileURLToPath(import.meta.url))


/*
 * ---------------- the tenant's data, which now lives in PostgreSQL ----------------
 *
 * `db.json` is still in the repo and still generated by the ingest scripts, but it is a
 * **seed** now, not the store: `npm run db:seed` loads it into the database and the
 * server never reads it again. `mock-server/db/model.mjs` is the shape it takes there —
 * 76 tables and 38 foreign keys, so an edge cannot name a node that is not on the
 * canvas and an audience cannot name a persona `auth_roles` has never heard of.
 *
 * **What did not change is the shape in memory.** `loadDb()` reads every table once and
 * rebuilds the same document every route already closes over, and `commitDb` writes the
 * whole thing back in one transaction. Nine thousand lines of handlers, `validateDb` and
 * every `check-docs` claim are untouched — because what makes this relational is the
 * schema and its constraints, not whether each handler writes its own SELECT.
 *
 * The consequence worth knowing before debugging: **editing `db.json` now does nothing
 * until you re-seed.** That is the stale-server trap one layer down, so the boot banner
 * says which database answered and how many rows it held.
 */
async function readDbOrExit() {
  const refuse = (lines) => {
    console.error(`\nmock-server: refusing to start — ${lines}\n`)
    process.exit(1)
  }

  let state
  try {
    state = await isSeeded()
  } catch (error) {
    refuse(`${connectionAdvice(error)}`)
  }

  /*
   * "The database is not there", "the schema is not applied" and "the schema is applied
   * but empty" are three different problems with three different one-line fixes, and a
   * server that answered all three with an empty document would send every one of them
   * to the wrong place. Each is named.
   */
  if (!state.migrated) {
    refuse(
      `there is no "${DB_SCHEMA}" schema on ${dbTarget()}.\n` +
        '  Apply it and load the seed:\n      npm run db:migrate && npm run db:seed',
    )
  }
  if (state.tables !== tableOrder().length) {
    refuse(
      `the "${DB_SCHEMA}" schema on ${dbTarget()} has ${state.tables} tables, but the model has ` +
        `${tableOrder().length}.\n  It was applied before a model change. Rebuild it:\n      npm run db:migrate && npm run db:seed`,
    )
  }
  if (!state.seeded) {
    refuse(
      `the "${DB_SCHEMA}" schema on ${dbTarget()} is empty.\n` +
        '  Load mock-server/db.json into it:\n      npm run db:seed',
    )
  }
...
/**
 * Writes the whole document to PostgreSQL in one transaction, then hot-swaps the
 * in-memory copy in place — every route closes over `db`, so mutating it is what makes
 * the edit take effect without a restart.
 *
 * **The order of the two steps is still the load-bearing part.** Validation and the
 * in-memory swap both happen *synchronously*, before the first `await`; only the write
 * yields. That is what keeps a second handler from reading a stale document: every call
 * site builds its `next` from `db` and calls straight into here, so by the time anything
 * else can run, `db` already holds the new state. Swapping after the write instead would
 * open a window where two overlapping edits each read the pre-edit document and the
 * second silently dropped the first.
 *
 * The cost of swapping first is that a failed write would leave memory ahead of the
 * database, so the previous document is kept and restored if the write rejects — the
 * caller gets the error *and* the two agree again.
 *
 * **What the transaction replaced.** Writing `db.json` had to chain its writes per path
 * by hand, because two commits shared one temp file and the file that landed would have
 * been neither document. `BEGIN` … `COMMIT` is that guarantee and the rollback in one,
 * and it adds the one a rename could not give: a failure cannot leave 40 of the 76
 * tables written.
 */
async function commitDb(next) {
  ...
  if (problems.length > 0) {
    throw new Error(
      `refusing to write — ${problems.join('; ')}. If this server has ` +
  ...
  try {
    ...
    /*
     * The constraint that refused is the interesting part of the message, not the fact
     * that a write failed: `canvas_edges_from_node_fkey` names the exact element the
     * document invented. `connectionAdvice` passes it through and only replaces the
     * unreadable connection codes.
     */
    throw new Error(
      `could not write to the "${DB_SCHEMA}" schema — ${connectionAdvice(error)}. Nothing was ` +
        'changed; the transaction rolled back and the in-memory document has been put back ' +
        'the way it was.',
    )
  }
}
...
const routes = [
...

  {
    ...
    handle: (_req, res) =>
      send(res, 200, {
        /*
         * Where the document actually is, which is no longer a file. The editor prints
         * this, and printing `mock-server/db.json` would send an editor to a seed the
         * server has not read since boot — the file it names has to be the one a save
         * lands in.
         */
        path: `postgres://${dbTarget()} · schema "${DB_SCHEMA}" · ${tableOrder().length} tables`,
...
if (startupProblems.length > 0) {
  console.error(`\nmock-server: refusing to start — the "${DB_SCHEMA}" schema on ${dbTarget()} cannot be served.`)
  for (const problem of startupProblems) console.error(`  · ${problem}`)
  /*
   * The fix is a re-seed, not a file restore. `db.json` is the seed and the ingest
   * scripts own it, so pointing at `git show` here would send somebody to repair a file
   * the server no longer reads — and the database would still hold the broken document.
   */
  console.error(
    '\n  The document in the database is not the one db.json holds. Restore the seed and' +
      '\n  load it again:' +
      '\n      git checkout HEAD -- mock-server/db.json' +
      '\n      npm run db:seed\n',
  )
  process.exit(1)
}
...
server.listen(PORT, () => {
  ...
  /*
   * Which database answered, said first, because "I edited db.json and nothing changed"
   * is now the most likely confusion in this repo — the file is a seed and this process
   * has not read it since boot. Naming the schema and the row count turns that into a
   * one-line check against `npm run db:seed`.
   */
  console.log(
    `  data from postgres://${dbTarget()} · schema "${DB_SCHEMA}" · ` +
      `${tableOrder().length} tables (mock-server/db.json is the seed — re-seed after editing it)`,
  )
  console.log(
